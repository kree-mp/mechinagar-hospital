...
import { ServiceCategory } from '@/lib/db/models';
import { requireCmsAuth } from '@/lib/auth/requireCmsAuth';
import { revalidatePublic } from '@/lib/cache';
import { serviceCategoryPatchSchema } from '@/lib/validations/cms';
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = serviceCategoryPatchSchema.safeParse(body);
  if (!parsed.success) {
