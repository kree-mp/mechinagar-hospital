...
import { DownloadCategory } from '@/lib/db/models';
...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const parsed = downloadCategorySchema.partial().safeParse(body);
  if (!parsed.success) {
  ...
  const category = await DownloadCategory.findByIdAndUpdate(
    id,
